document.addEventListener('DOMContentLoaded', () => {
    ...
    overrideBtn.addEventListener('click', async () => {
        try {
            ...
            console.log('[Override] Starting override process for:', blockedUrl);
            
            // Send override request to disable blocking
            ...
            console.log('[Override] Sending override request');
            
            const response = await browser.runtime.sendMessage(overrideRequest);
            
            if (response && response.success) {
                console.log('[Override] Override granted, preparing to redirect');
                
                // Update button to show success
                overrideBtn.textContent = "Access granted! Redirecting...";
                overrideBtn.style.background = '#27ae60';
                
                // Wait longer to ensure blocking is fully disabled
                // This is critical on Android where async operations are slower
                ...
                const errorMsg = response && response.error ? response.error : 'Override request failed';
